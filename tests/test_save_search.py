...
def example_json_path():
    ...
    return script_dir / "modules" / "job_search" / "example_agent_output.json"
...
def save_search_script_path():
    ...
    script_dir = Path(__file__).parent.parent
    return script_dir / "modules" / "job_search" / "save_search.py"
...
def test_save_search_file_naming(
    ...
    for file_path_str in files_created:
        ...
        assert file_name.startswith(expected_pattern), (
            f"File {file_name} doesn't match expected pattern"
        )
        assert file_name.endswith((".json", ".md")), (
            f"File {file_name} doesn't have .json or .md extension"
        )
