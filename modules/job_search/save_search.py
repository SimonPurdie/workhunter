    ...
    try:
        ...
        with open(json_path, "r") as json_file:
            ...
            if result.returncode != 0:
                print(
                    f"Warning: Markdown generation failed - {result.stderr}",
                    file=sys.stderr,
                )
            else:
        ...
        print(json.dumps({"error": "invalid_json", "message": str(e)}), file=sys.stderr)
        sys.exit(1)
    except Exception as e:
        print(json.dumps({"error": "save_failed", "message": str(e)}), file=sys.stderr)
        sys.exit(1)
